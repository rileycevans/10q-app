...
export function QuestionPageClient({ index }: { index: number }) {
  ...
  useEffect(() => {
    ...
    if (attempt.current_index !== questionIndex) {
      // Clamp. A static export only contains pages 1..MAX_QUESTIONS_PER_QUIZ,
      // so pushing an out-of-range index is a 404 inside the app bundle
      // rather than a soft redirect the way it is on the web.
