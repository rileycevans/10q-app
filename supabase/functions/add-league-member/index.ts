/**
 * Add League Member Edge Function (Standalone)
 * Adds a member to a league by handle (owner only)
 */
...
Deno.serve(async (req) => {
  ...
  logStructured(requestId, "add_league_member_request", {});
  ...
  try {
    ...
    if (membershipError || !membership || membership.role !== "owner") {
      return errorResponse(
        ErrorCodes.NOT_AUTHORIZED,
        "Only league owners can add members",
        requestId,
        403
      );
    }
    ...
    if (profileError || !profile) {
      return errorResponse(
        ErrorCodes.VALIDATION_ERROR,
        `Player with handle "${handle}" not found`,
        requestId,
        404
      );
    }
    ...
    if (addError) {
      logStructured(requestId, "add_league_member_error", {
    ...
    logStructured(requestId, "add_league_member_success", {
      league_id,
      player_id: profile.id,
