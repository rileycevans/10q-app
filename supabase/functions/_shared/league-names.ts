/**
 * League name validation (for Edge Functions).
 *
 * Deno cannot import the @10q/contracts workspace package, so this mirrors
 * packages/contracts/src/league-names.ts. The two must stay in sync: the
 * client check is a convenience, this copy is what actually enforces the rule.
 * Tests for the shared logic live with the contracts copy.
 */

import { containsBlockedContent } from "./handle-blocklist.ts";
