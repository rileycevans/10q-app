-- ---------------------------------------------------------------------------
-- record_claim: the heart of step 7.
-- Resolves destination club (lazy-create), source club (optional, lazy-create),
-- player (lazy-create), and journalist (by handle). Upserts the canonical claim
-- on (player, destination, window) advancing stage forward only for
-- non-contradicting reports, then inserts one claim_report (deduped by url).
--
-- Returns the claim_id, report_id, whether each entity was newly created, and
-- a status: 'recorded' | 'duplicate_report'.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Map stage name -> id/rank (NULL stage is allowed on the report)
  ...
  -- Upsert the canonical claim on the natural key
  ...
  ELSE
    ...
    UPDATE transfers.claims SET
      ...
      -- advance stage forward only, and only for non-denials
      ...
      END,
      -- fill in source club / transfer type if previously unknown
  ...
  -- Insert the report (dedupe on source_url)
  BEGIN
  ...
  EXCEPTION WHEN unique_violation THEN
    v_dup := true;
    SELECT id INTO v_report_id FROM transfers.claim_reports WHERE source_url = p_source_url;
  END;
