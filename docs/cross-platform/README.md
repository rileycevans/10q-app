...
**1. There are live security defects, and they are now preconditions 0B and 0C — not cleanup.** `delete-attempt` lets any signed-in user wipe their attempt and replay it with the answer key in hand, for a perfect 100 every day — stopped today only by a client-side `if (!isAdmin)`. `publish-quiz` is an unauthenticated service-role write. Packaging the app makes both trivially discoverable. They are not "Capacitor migration work", but shipping an IPA/APK turns hidden client behavior into something anyone can unzip and read. **They sit behind the 0E gate, before any native project exists.**

**2. Google Play production access may be a four-week gate — run it as a parallel external track from day one.** If the Play account is a personal account created after 2023-11-13, production requires a closed test with **12 testers opted in for 14 consecutive days**, then a human-reviewed written application. This is the single largest schedule risk in the program and it blocks nothing else — **start it first.** See [ANDROID.md](release/ANDROID.md) §4.4.

**3. Precondition 0A has not been run, and it is the architectural go/no-go.** The static export's behavior inside a real WebView — specifically Next's export-mode `HEAD` probe through Capacitor's scheme handler — is the one assumption the audit could not verify. If it fails, in-flight quiz state is destroyed on every question transition, and the architecture needs revising **before** a week of downstream work. **Do not create `ios/`, `android/`, or add any `@capacitor/*` dependency until the 0E gate clears.** See [05-migration-plan.md](05-migration-plan.md).
